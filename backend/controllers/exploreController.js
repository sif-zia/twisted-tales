...
const getLatestStories = async (req, res) => {
    try {
        const latestStories = await Story.find().sort({ createdAt: -1 }).limit(10); 
...
const getWriterOfTheMonth = async (req, res) => {
    try {
        ...
        if (writerOfTheMonth) {
            res.status(200).json({ writerOfTheMonth });
        } else {
...
module.exports = {getLatestStories, getTrendingStories, getGenres, getTopPicks, getWriterOfTheMonth, getGenreCover};